...
class Cons1(rclpy.node.Node):
    ...
    def run_(self):
        if self.explore_robot.position is not None and self.goals is not None:        
            if self.explore_robot.is_takeoff_completed():
                if self.explore_robot.is_reached(): 
                    print(f"goal is {self.goals[self.goal_idx]}")
                    self.explore_robot.go_to(goal=self.goals[self.goal_idx])
                    self.goal_idx+=1
